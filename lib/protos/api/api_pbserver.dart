...
abstract class DgraphServiceBase extends GeneratedService {
  ...
  $async.Future<TxnContext> commitOrAbort(ServerContext ctx, TxnContext request);
  $async.Future<Version> checkVersion(ServerContext ctx, Check request);

  GeneratedMessage createRequest(String method) {
    switch (method) {
      case 'Query': return new Request();
      case 'Mutate': return new Mutation();
      case 'Alter': return new Operation();
      case 'CommitOrAbort': return new TxnContext();
      case 'CheckVersion': return new Check();
  ...
  $async.Future<GeneratedMessage> handleCall(ServerContext ctx, String method, GeneratedMessage request) {
    switch (method) {
      ...
      case 'Alter': return this.alter(ctx, request);
      case 'CommitOrAbort': return this.commitOrAbort(ctx, request);
      case 'CheckVersion': return this.checkVersion(ctx, request);
      default: throw new ArgumentError('Unknown method: $method');
    }
  }
